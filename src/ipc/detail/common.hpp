 ...
 *   - This defines the `enum class Log_component` itself, auto-generated via macro magic.
 *   - This declares (but does not yet populate) an (Log_component -> std::string) multimap containing
 *     each enum value's string representation.
 *     - common.cpp will complete this procedure by populating the map.
 *
 * IMPORTANT:
 * `ipc/detail/macros/log_component_enum_declare.macros.hpp` declares the log components used by all of Flow-IPC
 * (i.e., by everything in ::ipc namespace).  Hence, if enum value X is in that file, then it can be (a)
...
#undef FLOW_LOG_CFG_COMPONENT_ENUM_CLASS
#undef FLOW_LOG_CFG_COMPONENT_ENUM_NAME_MAP
#define FLOW_LOG_CFG_COMPONENT_ENUM_CLASS Log_component
/* This tells the below and certain other boiler-plate macro files the name of the enum-to-string multimap that maps
 * each enum value to its name for log output and config purposes. */
#define FLOW_LOG_CFG_COMPONENT_ENUM_NAME_MAP S_IPC_LOG_COMPONENT_NAME_MAP
